...
const DetailedGraphsPage = ({ initialType = "temperature", theme, lang }) => {
  ...
  const [stats, setStats]       = useState({ min: "—", max: "—", avg: "—", current: "—" });

  // ── Tipos de sensor — grid 3×3 ──
  const TYPES = [
    // linha 1
    { key: "temperature",      short: t(lang, "temperature"),      unit: "°C",                      color: theme.accentOrange,  icon: "thermometer",   note: null },
    { key: "soil_humidity",    short: t(lang, "soil_humidity"),    unit: "%",                       color: theme.accentBlue,    icon: "water",         note: null },
    { key: "air_humidity",     short: t(lang, "air_humidity"),     unit: "%",                       color: theme.accentPurple,  icon: "cloud",         note: null },
    // linha 2
    { key: "water_flow_sensor",short: t(lang, "water_flow_sensor"),unit: t(lang, "water_flow_unit"),color: theme.accentBlue,    icon: "water-outline", note: null },
    { key: "rain",             short: t(lang, "rain"),             unit: "%",                       color: theme.info,          icon: "rainy-outline", note: t(lang, "rain_note") },
    { key: "light",            short: t(lang, "light"),            unit: "%",                       color: theme.accentAmber,   icon: "sunny-outline", note: t(lang, "light_note") },
  ];

  useEffect(() => setTypeKey(initialType), [initialType]);

  const isFlowSensor = typeKey === "water_flow_sensor";
  const flowMeta     = isFlowSensor ? getFlowMeta(period.hours, lang) : null;
  ...
  const buildChartData = () => {
    ...
    return {
      labels:   sampled.map(({ timestamp }) => formatLabel(timestamp)),
      datasets: [
        {
      ...
      <ScrollView contentContainerStyle={s.content}>
              ...
              {row.map((tp) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      s.typeTab,
                      on && { borderColor: tp.color, backgroundColor: theme.bgCardAlt },
                    ]}
        ...
        <View style={[s.heroCard, { borderLeftColor: color }]}>
          <View style={{ flex: 1 }}>
            ...
            <View style={s.heroValueRow}>
              <Text style={[s.heroValue, { color }]} numberOfLines={1} adjustsFontSizeToFit>
                {stats.current}
              </Text>
              <Text style={[s.heroUnit, { color }]}> {displayUnit}</Text>
            </View>
            {note ? (
              <Text style={[s.noteText, { color }]}>{note}</Text>
            ) : null}
          </View>
          <View style={[s.heroIcon, { backgroundColor: withOpacity(color, 0.15) }]}>
        ...
        <View style={s.chartCard}>
          ...
          {loading ? (
            ...
            <View style={s.noData}>
              <Ionicons name="bar-chart-outline" size={44} color={theme.textFaint} />
...
const styles = (theme) =>
  StyleSheet.create({
    ...
    noDataText: { fontSize: 16, fontWeight: "600", color: theme.textMuted, marginTop: 12 },
    noDataSub:  { fontSize: 12, color: theme.textFaint, textAlign: "center", marginTop: 6 },
  });
