...
// Limiar de chuva (%) para sugerir pausar a irrigação: acima disso = chovendo.
// Se o seu sensor for invertido (valor CAI quando molha), troque a comparação
// no efeito de alerta de chuva para `<=` e ajuste este valor.
...
const App = () => {
  ...
  const rainAlertedRef = useRef(false); // evita repetir o alerta na mesma chuva
  ...
  const handleIoTMessage = useCallback(
    (msg) => {
      ...
      // litros_min = vazão real medida (L/min). O campo "vazao" é ignorado.
      if (msg.litros_min !== undefined)
        patch.water_flow_sensor = parseFloat(msg.litros_min);
    ...
    try {
      iotSub.current = iotService.subscribe(
        ...
        (err) => {
          ...
          console.warn("[App] ⚠️ MQTT interrompido:", err?.message || err);
        },
      );
    } catch (err) {
      console.error("[App] ❌ Erro ao inscrever:", err);
    }
    return () => {
  ...
  // ── Alerta de chuva: sugere pausar a irrigação quando detecta chuva ──
  // Só dispara na BORDA (quando a chuva sobe o limiar), independente de estar
  // irrigando ou não. O botão "Parar irrigação" desliga a bomba se necessário.
  useEffect(() => {
    const rain = pivotData?.sensors?.rain;
    if (rain == null) return;

    if (rain >= RAIN_THRESHOLD && !rainAlertedRef.current) {
      rainAlertedRef.current = true;
      Alert.alert(t(lang, "rain_alert_title"), t(lang, "rain_alert_msg"), [
        { text: t(lang, "rain_alert_dismiss"), style: "cancel" },
        {
          text: t(lang, "rain_alert_stop"),
          style: "destructive",
          onPress: () => handleChangeFlow(0),
        },
      ]);
    } else if (rain < RAIN_THRESHOLD) {
      rainAlertedRef.current = false; // chuva passou → permite alertar de novo
    }
  }, [pivotData?.sensors?.rain, lang, handleChangeFlow]);
            ...
            {setorizacaoAtiva && (
              <View style={st.setorizacaoBadge}>
                <Ionicons
                  ...
                  color={theme.primary}
                />
                <Text style={st.setorizacaoBadgeText}>
                ...
                .map((key) => {
                  ...
                  return (
                    <StatusCard
                      ...
                      isAdjustable={
                        isConnected && !isRotating && !setorizacaoAtiva
                      }
                      theme={theme}
                    />
                  );
